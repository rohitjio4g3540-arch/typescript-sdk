 ...
 * - the F5 conversion guard: a UrlElicitationRequiredError escaping a handler
 *   on the modern era becomes a URL-mode elicitation inside an input-required
 *   result (capability-gated) — `-32042` never reaches the 2026-07-28 wire —
 *   while 2025-era traffic keeps today's `-32042` behavior;
...
describe('input-required returns on the 2026-07-28 era', () => {
    ...
    it('prompts/get and resources/read handlers can return input_required (no catch-all rewraps it)', async () => {
        const server = new McpServer(
            ...
            { capabilities: { prompts: {}, resources: {} }, eraSupport: 'dual-era' }
        );
...
describe('guards', () => {
    ...
    it('checks every embedded request against the capabilities the request itself declared (-32003 on violation)', async () => {
        ...
        server.registerTool('open-url', { inputSchema: z.object({}) }, async () =>
            inputRequired({
                inputRequests: {
                    auth: inputRequired.elicitUrl({ message: 'Sign in', elicitationId: 'auth-1', url: 'https://example.com' })
                }
            })
        );
        const { request, close } = await wire(server);
    ...
    it('non-multi-round-trip methods can never emit input_required (server-bug guard)', async () => {
        const server = new Server({ name: 's', version: '1.0.0' }, { capabilities: { completions: {} }, eraSupport: 'dual-era' });
...
describe('F5 conversion guard (UrlElicitationRequiredError)', () => {
    const URL_PARAMS = { mode: 'url' as const, message: 'Sign in to continue', elicitationId: 'elicit-7', url: 'https://example.com/auth' };

    function buildUrlThrowingServer(eraSupport?: 'dual-era') {
        const server = new McpServer(
            { name: 's', version: '1.0.0' },
            { capabilities: { tools: {} }, ...(eraSupport ? { eraSupport } : {}) }
        );
    ...
    it('converts to a URL-mode elicitation inside input_required when the request declares elicitation.url', async () => {
        const { request, close } = await wire(buildUrlThrowingServer('dual-era'));
        ...
        expect(result.inputRequests).toEqual({ 'elicit-7': { method: 'elicitation/create', params: URL_PARAMS } });
        expect(JSON.stringify(answer)).not.toContain('-32042');

        await close();
    });

    it('fails loudly (never -32042) on the modern era when the request does not declare elicitation.url', async () => {
        const { request, close } = await wire(buildUrlThrowingServer('dual-era'));
...
describe('push-style APIs on 2026-era requests', () => {
    it('ctx.mcpReq.elicitInput rejects before any wire traffic and the catch-all surfaces the inputRequired() steer as isError', async () => {
        ...
        const { request, inbound, close } = await wire(server);
